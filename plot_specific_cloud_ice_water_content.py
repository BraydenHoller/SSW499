...
#     # Determine pixel size in y (assumes north-up image)
...
# # Here we assume that the SSW reanalysis dataset contains the required variables.
...
# # Each event is represented as a dictionary; here we reuse ds_ssw for all events.
# all_events = []
...
# pressure_level = 925  # Specify the pressure level (hPa) to plot
...
#         ds = event['ds']
#         try:
#             # Select the specific cloud ice water content.
...
#     # Update the colorbar using the first mappable (if any)
#     if mappables:
#         cax.cla()
...
# ani.save('ciwc_permafrost_animation_SSW2.mp4', writer=writer)
...
matplotlib.rcParams['animation.ffmpeg_path'] = r"C:\ffmpeg\ffmpeg\ffmpeg-7.1-full_build\bin\ffmpeg.exe"
writer = FFMpegWriter(fps=1, metadata=dict(artist='Brayden Holler'), bitrate=1800)
ani.save('ciwc_permafrost_animation_SSW3.mp4', writer=writer)
