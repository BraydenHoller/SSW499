...
snowfall_thinned_true_cum = snowfall_thinned_true.cumsum(dim='valid_time')
...
def update(frame):
    ...
    data = snowfall_thinned_true_cum.isel(valid_time=frame)
    data.plot.pcolormesh(
        ...
        add_colorbar=False,
        vmin=0,
        vmax=snowfall_thinned_true_cum.max()
    )
...
ani.save('us_snowfall_animation0722-0723cumulative.mp4', writer=writer)


# Display the animation in a Jupyter notebook
from IPython.display import Video

Video('us_snowfall_animation0722-0723cumulative.mp4', embed=True)